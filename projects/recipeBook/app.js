...
const recipeDashboard = document.querySelector('.recipe-dashboard');
...
void function displayRecipes(){
  ...
  for(i=0; i<data.length; i++){
    ...
    typeDiv.classList.add('dish-type');
    const typeCard = document.createElement('p');
    typeCard.innerText = data[i].type + ' - ' + data[i].main_ingredient;
    ...
    recipeCard.innerText = data[i].title;
    const imageCard = document.createElement("IMG");
    imageCard.setAttribute("src", data[i].img);
    ...
    for(j=0; j<data[i].ingredients.length; j++){
      ...
      ingredient.innerText = data[i].ingredients[j].ingredient;
    ...
    for(j=0; j<data[i].instructions.length; j++){
      const instruction = document.createElement('li');
      instruction.innerText = data[i].instructions[j].content;
      instructionsDiv.appendChild(instruction);
    }

    recipeDiv.appendChild(typeCard);
    recipeDiv.appendChild(ingredientDiv);
    recipeDiv.appendChild(instructionsDiv);
    recipeDashboard.appendChild(recipeDiv);
